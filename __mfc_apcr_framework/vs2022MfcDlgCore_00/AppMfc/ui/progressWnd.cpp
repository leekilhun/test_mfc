...
CprogressWnd::CprogressWnd(bool bPos /*=true*/)
  : m_iDisplayType(PERCENT)
{
  m_hbrush = NULL;
  m_MostTopFlag = false;
  m_bCenter = bPos;
...
CprogressWnd::CprogressWnd(CWnd* pParent, LPCTSTR pszTitle, bool bPos/*=true*/, bool bSmooth /* = true */)
  : m_iDisplayType(PERCENT)
{
  m_hbrush = NULL;
  m_MostTopFlag = false;
...
BOOL CprogressWnd::Create(CWnd* pParent, LPCTSTR pszTitle, bool bSmooth /* = true */)
{
  bool bSuccess;
...
int CprogressWnd::SetPos(int nPos)
{
  ...
  if (nPercentage != m_nPrevPercent)
  {
    ...
    strTitle.Format(_T("%s [%d%%]"), m_strTitle.GetBuffer(), nPercentage);
...
void CprogressWnd::SetText(LPCTSTR fmt, ...)
{
  ...
  va_start(args, fmt);
  _vstprintf_s(buffer, _countof(buffer), fmt, args);
  va_end(args);
...
void CprogressWnd::SetDisplayType(int iType)
{
  m_iDisplayType = iType;
}


void CprogressWnd::PeekAndPump()
{
  //	SetFocus();
