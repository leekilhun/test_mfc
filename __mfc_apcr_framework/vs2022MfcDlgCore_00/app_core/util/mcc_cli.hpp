 ...
 * \file   mcc_cli.hpp
 * \brief
 *         Simple CLI library using console input and output
 *
...
namespace TinyC
{
  ...
  public:
    ...
    static void registerCallback(std::string key, std::function<int(int, char**)> cb)
    {
      callbacks[key] = cb;
    }

    static int split(char* arg_str, char** argv, const char* delim_chars, std::size_t max = 1024)
    {
      int argc = 0;
      char* tok = nullptr;
  ...
  static COORD get_cursor_position()
  {
  ...
  static void set_cursor_position(int x, int y)
  {
    ...
    static std::string cli_to_upper(std::string_view str)
    {
      std::string s(str);
    ...
    static int cli_show_list(int argc, char* argv[])
    {
