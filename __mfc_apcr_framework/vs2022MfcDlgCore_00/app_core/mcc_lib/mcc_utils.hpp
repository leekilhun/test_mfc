 ...
 * \file   mcc_utils.hpp
 * \brief
 *  2024.06.05  filepath, filename, file extension 관련 함수 추가
 *        7.22  now format 추가
 *        8.09  기간이 지난 파일 삭제 함수 추가
 *
 *
...
#ifndef GBETWEEN
#define GBETWEEN(value, low, high) (((value) < (low)) || ((value) > (high)))
#endif

#ifndef mcC_inLine
#define mcC_inLine inline
#endif
...
namespace crc
{
  ...
  mcC_inLine int crc16_modbus_cal(uint8_t *p_data, uint16_t length)
  {
    uint8_t nTemp;
  ...
  mcC_inLine void crc16_modbus_update(uint16_t *crc_in, uint8_t data)
  {
  ...
  mcC_inLine void crc16_update(uint16_t *crc_in, uint8_t data)
  {
    constexpr uint16_t poly = 0xA001;
    uint16_t           crc  = *crc_in;
    uint8_t            i;
...
namespace TinyC
{
  ...
  {
  public:
    explicit Que(std::size_t length = 4096) :
    m_Length(length),
    m_Head(0),
    m_Tail(0),
    m_Buff(length) {}
    ...
    inline void Clear()
    {
      ...
      if constexpr (std::is_same_v<T, std::string>)
      {
        m_Buff.clear();          // 벡터 재할당 (기존 요소 소멸)
        m_Buff.resize(m_Length); // 새 요소 기본 생성
      }
    ...
    // /**
    //  * @brief
    //  *  8.19 Shift the buffer to remove the first element in the buffer.
    //  * @return * void
    //  */
    // inline void Shift(){
    //   if (m_Tail != m_Head) {
...
namespace trans
{
  mcC_inLine size_t string_split(const std::string &str, std::vector<std::string> &result, const std::string &delimiter)
  {
    std::string        token;
  ...
  mcC_inLine size_t string_split(std::string_view s, std::vector<std::string> &result, char delimiter = ',')
  {
    std::stringstream ss{};
    std::string       item;
  ...
  mcC_inLine int split(char *arg_str, char **argv, const char *delim_chars, size_t max = 1024)
  {
    int   argc     = 0;
    char *tok      = nullptr;
...
namespace tim
{
  ...
  static auto &now = high_resolution_clock_t::now; //

  mcC_inLine unsigned int seed()
  {
    return static_cast<unsigned int>(high_resolution_clock_t::now().time_since_epoch().count());
  }

  // used to initialize random engine
  mcC_inLine unsigned int get_current_tick()
  {
    using std::chrono::duration;
    using std::chrono::duration_cast;
    auto current = duration_cast<std::chrono::milliseconds>(now().time_since_epoch());
    ...
    const time_point_t &end_time)
  {
    ...
    {
      auto period_of_time = duration_cast<std::chrono::duration<double, second_t>>(end_time - start_time);
      return period_of_time.count();
    }
    else
    {
      auto period_of_time = duration_cast<std::chrono::duration<double, TimeUnit>>(end_time - start_time);
...
name: test led, duration time: 200ms
  ...
  //2024-Jul-22 [Mon] : <19:46:15>
  mcC_inLine std::string get_now()
  {
    ...
    std::stringstream ss;
    ss << std::put_time(&local_time, "%Y-%b-%d [%a] : <%H:%M:%S> ");
    return ss.str();
  }

  //[2022-10-04 13:45:51'11]
  mcC_inLine void NowStr(char *p_str, int max_len)
  {
    auto now = std::chrono::system_clock::now();
  ...
  mcC_inLine unsigned int millis(void)
  {
    //return 0;
    using std::chrono::duration;
    using std::chrono::duration_cast;
  ...
  mcC_inLine unsigned int micros(void)
  {
    using std::chrono::duration;
    using std::chrono::duration_cast;
    auto current = duration_cast<std::chrono::microseconds>(now().time_since_epoch());

    return static_cast<unsigned int>(current.count());
  }

  mcC_inLine void delay(unsigned int ms)
  {
...
namespace file
{
  ...
  mcC_inLine void delete_old_files(const std::string &folder_path, int days, const std::set<std::string> &extensions)
  {
    ...
    try
    {
      for (const auto &entry : fs::recursive_directory_iterator(folder_path))
      {
        ...
        else if (entry.is_regular_file())
        {
          ...
          // Get the file extension and check if it's in the allowed extensions set
          std::string extension = entry.path().extension().string();
          if (extensions.find(extension) == extensions.end())
            continue; // Skip files with extensions not in the set

          auto last_write_time = fs::last_write_time(entry);
          auto file_time       = chrono::time_point_cast<chrono::system_clock::duration>(
            last_write_time - fs::file_time_type::clock::now() + now);
          auto age = chrono::duration_cast<chrono::hours>(now - file_time).count() / 24;

          if (age > days)
          {
            fs::remove(entry);
            deleted_file_count++;
            ss << "\t Deleted: " << ws2s(entry.path().wstring()) << "\n";
            //std::cout << "Deleted: " << ws2s(entry.path().wstring()) << std::endl;
          }
        }
      }
    }
    ...
    ss << "\t Search Folder   : " << folder_path << "\n";
    ss << "\t Subfolders Count: " << subfolder_count << "\n";
    ss << "\t Files Searched  : " << file_count << "\n";
    ss << "\t Files Deleted   : " << deleted_file_count << "\n";
    ss << "\t Execution Time  : " << duration << "(ms)" << "\n";
    ss << "\t ################################################################  \n\n";
    // std::cout << "Search Folder: " << folder_path << std::endl;
    // std::cout << "Subfolders Count: " << subfolder_count << std::endl;
    // std::cout << "Files Searched: " << file_count << std::endl;
    // std::cout << "Files Deleted: " << deleted_file_count << std::endl;
    ...
    std::string folder_path = "C:\\_github\\wsOutPacking\\project\\software\\mfc\\vs2022_mfc_3DApp\\tapingOutBox\\Log"; 
    int days =5; // For example, delete files older than 5 days

     // Set of extensions to delete
    std::set<std::string> extensions = { ".log", ".txt", ".exe" }; //

    delete_old_files(folder_path, days, extensions);
  ...
  /**
  * @brief add_suffix_before_extension
  * 
  * @param filename 
  * @param suffix 
  * @return std::string 
  */
  mcC_inLine std::string add_suffix_before_extension(const std::string &filename, const std::string &suffix)
  {
    ...
    {
      new_filename.insert(idx, suffix);
    }
  ...
  /**
  * @brief make fold and copy file to fold
  * 
  * @param fold_nanme 
  * @param ref_path_filename 
  * @return std::string 
  */
  mcC_inLine std::string make_fold_copyfile_cwd(const std::string &fold_nanme, const std::string &ref_path_filename)
  {
    fs::path    refer_path(ref_path_filename);
    fs::path    refer_parent_path = refer_path.parent_path();
    std::string refer_file_name   = refer_path.filename().string();
    std::string refer_dir_name    = refer_parent_path.string();
    std::string full_path         = refer_path.string();
    // std::cout << "cfg_dir_name: " << cfg_dir_name << std::endl;
    // std::cout << "cfg_file_name: " << cfg_file_name << std::endl;
    // std::cout << "full_path: " << full_path << std::endl;

    fs::path cwd      = fs::current_path();
    fs::path make_dir = cwd / fold_nanme;

    if (!fs::exists(make_dir))
      fs::create_directory(make_dir);

    fs::path def_file = ref_path_filename;
    fs::path new_file = make_dir / refer_file_name;

    if (fs::exists(new_file))
    {
      std::cout << "File already exists" << std::endl;
      fs::path backup_file = refer_parent_path / add_suffix_before_extension(refer_file_name, "_backup");
      fs::copy(def_file, backup_file, fs::copy_options::overwrite_existing);
      fs::copy(new_file, def_file, fs::copy_options::overwrite_existing);
    }
    else
    {
      std::cout << "File does not exist" << std::endl;
      fs::copy(def_file, new_file);
    }

    return new_file.string();
  }

  mcC_inLine std::filesystem::path get_data_fold_path(void)
  {
#ifdef _USE_AFX_APP_CORE
    std::filesystem::path cwd = std::filesystem::current_path();//.parent_path() / "app_core";
#else
  ...
  mcC_inLine void CurrDir(char *p_buff, int len)
  {
  ...
  mcC_inLine void CurrDir(std::string &ret_str)
  {
    fs::path curr_path = fs::current_path();
  ...
  mcC_inLine int MakeDir(const char *dir_name)
  {
  ...
  mcC_inLine int DeleteDir(const char *dir_name)
  {
  ...
  mcC_inLine bool isFile(const fs::directory_entry &entry)
  {
    return fs::is_regular_file(entry);
  }

  mcC_inLine bool isDir(const fs::directory_entry &entry)
  {
    return fs::is_directory(entry);
  }

  mcC_inLine bool isDirExists(const char *dir)
  {
    return fs::exists(dir) && fs::is_directory(dir);
  }

  mcC_inLine bool isFileExists(const char *file)
  {
    return fs::exists(file) && fs::is_regular_file(file);
  }

  mcC_inLine int SearchDir(const char *dir_str)
  {
  ...
  mcC_inLine int GetFileCount(const char *dir_str, bool inc_dir)
  {
  ...
  mcC_inLine bool isHidden(const std::filesystem::path &path)
  {
    return path.filename().string().front() == '.';
  }

  mcC_inLine bool isLeafDir(const char *dir_str)
  {
  ...
  mcC_inLine int DeleteFilesInDir(const char *dir_str, bool inc_dir)
  {
    fs::path dir_path(dir_str);
    int      ret = FILE_ERR_SUCCESS;
  ...
  mcC_inLine bool FileCopy(const char *pdest, const char *psour)
  {
  ...
  mcC_inLine std::error_code CopyFilesInDir_binary(const char *dst_dir, const char *src_dir, bool over_write, bool inc_dir)
  {
  ...
  mcC_inLine int GetFileSize(const char *file_path)
  {
